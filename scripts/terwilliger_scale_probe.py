...
Walls:
  1. PSD-roundability — smallest precision P at which the float64 dual blocks round to exactly-PSD rationals.
     The ladder tops out at 1e14: with the SDPA-GMP duals that are the solve leg's default since the D6/Q-pit-2
     fix, (19,6) first rounds at P=1e12 (CLARABEL-era duals rounded at 1e8; the banked (23,6)/(25,10) LP certs
     needed 1e14 — docs/results/terwilliger-solve-leg-2026-07-02.md). Small cells still round at 1e6–1e8.
...
def probe(n, d, target,
          precisions=(10 ** 6, 10 ** 7, 10 ** 8, 10 ** 9, 10 ** 10, 10 ** 12, 10 ** 14)):
    ...
    # Restore over the FULL multiplier set, exactly as certify() does. A float-activity-restricted column
    # set (the CLARABEL-era shortcut) no longer spans the residuals: the SDPA-GMP duals sit much closer to
    # complementary slackness, so the needed columns fall below any activity tolerance.
    ...
    row.update({"residual_zero": chk["n_residuals_nonzero"] == 0,
...
def main() -> int:
    ...
    res = {"rows": rows,
           "reading": ("Path C scale probe. Confirms the exact A(19,6) certificate is COMPUTE-BOUND, not "
                       "impossible: the dual blocks round to PSD (P=1e8 for CLARABEL-era duals; P=1e12–1e14 for "
                       "the default SDPA-GMP duals), the min-norm restoration zeroes the "
