...
- **Lever A — multi-variable faithfulness certification.** The blocked claims are DSL-legal and Z3-*encodable*, so we first assumed they were "already Z3-decidable" — a cheap fix. **A follow-up measurement falsified that:** `decide_unsat([a≥0 ∧ b≥0, (a²+b²) % 4 == 3])` returns **`None` (unknown)** — two-variable nonlinear-modular UNSAT is *outside* Z3's decidable fragment, so the coverage probe DEFERs. But **full-period enumeration decides these exactly** where Z3 cannot — the *same* mechanism as Lever B. **So Lever A and Lever B unify:** one trust-critical increment (an exact-enumeration/periodicity faithfulness backend), not a cheap A and a gated B. See **ADR 0054**, now under adversarial review. *(The correction is itself a datapoint: the "cheap A" premise did not survive contact with measurement.)*
...
So the honest ordering is **[the unified exact-enumeration backend, reviewed] → prover**: only once richer claims can be *certified faithful* (ADR 0054) does prover reach (Leanstral, repair) become the limiting factor. Reviewers: read §1 below as the analysis behind the unified backend (ADR 0054), which the empirical finding turned from optional into the demonstrated binding constraint.
