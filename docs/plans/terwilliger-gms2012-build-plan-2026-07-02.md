...
Audit tier throughout; no trusted-surface edit; tests/test_invariants.py stays byte-identical.
-->
...
|---|---|---|---|
| **GATE 1** (discovery only) | Are there ≥3 open cells whose *current* record is Schrijver-2005-or-earlier AND still `lb < ub`? | Re-fetch Brouwer snapshot; attribute every (d∈{6,8,10,12}, n∈19..28) ub to its source paper (extend `docs/data/brouwer-snapshot-2026-07.json` provenance). | **≥3 such cells** (our evidence says ~0 outside the pre-SDP frontier). |
...
DISCOVERY needs GATE 1 ∧ GATE 2; AMPLIFICATION needs GATE 1′. A failed discovery gate does **not** kill the
build — it re-points it at the amplification objective, which the operator may still choose.
...
**Phase 1 — quadruple-β / block oracle (M, 2–3 d; HIGH reuse).** Extend the D1 `(k,l)` doubly-indexed machinery
— `scripts/terwilliger_cwc_beta.py::block_pairs`, `block_idx(w,v,k,l)`, `r_block(w,v,k,l,…)` — to the full-graph
GMS quadruple β `β^{t,w}_{i,j,k}·β^{s,v}_{i,j,l}`. **Exit test**: a real-code PSD differential test + a
corrupt-control (the D1/Phase-0 pattern), and a checked-in β oracle TSV. **Risk**: product-β magnitudes ≈ β²
(~90-bit numerators) — carry exact integers, never floats, in the oracle.
...
is projected ~3–5× three-point (unmeasured) — measure it here; if the tableau walls, the Bareiss/integer-simplex
...
## Risk register (the D3 walls, with mitigations)
...
3. **SDPA-GMP on β² conditioning.** Bound n early; empirical precision/normalization tuning (Phase 2).
4. **Exact-LP active-set growth.** Measure in Phase 4; Bareiss/integer-simplex fallback.
...
**Phase 0: 1–2 days.** **Phases 1–6: ~10–15 person-days / 4–6 weeks** on GO. Phases 1→2→3 are the critical path
(faithfulness gate); 4→5 reuse proven legs; 6 is the payoff. Kill points: GATE 2 (discovery), Phase 3
(faithfulness), Phase 5 one-slice benchmark (kernel). External-review brief for this plan:
`docs/briefs/terwilliger-gms2012-external-review-brief-2026-07-02.md`.
